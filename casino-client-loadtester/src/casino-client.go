...
func main() {
    ...
    delayInMsPtr := flag.Int("d", 10, "Delay in ms between each thread at startup.")
    secondsPtr := flag.Int64("s", 60, "Number of seconds to run simulation.")
    flag.Parse()
    ...
    for i := 0; i < *usercountPtr; i++ {
      ...
      time.Sleep(time.Duration(*delayInMsPtr) * time.Millisecond)
    }
